...
- **How**：导入弹窗双 Tab：① Excel（现有）；② **从【米多公司默认项目】导入**（无其它业务项目选择器）→ 树勾选 → 确认。  
...
- 选择模式：`CASE_IDS` / `MODULE_IDS` / `FOLDER_IDS` / `ALL` / `UNPLANNED` + `SKIP`/`OVERWRITE`。  
...
### 3.10 【#10】选全部 / 未规划后显示导入按钮

与 #5 同弹窗；`all | unplanned | 有勾选` → 导入按钮可用。受 §3.5 批量上限约束。
...
| T8 | ALL/未规划；501 条 | ≤500 成功；超限拒绝；失败整单回滚 |
| T9 | 建项进入 | 权限即时生效 |
